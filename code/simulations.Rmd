...
# save(A,ps_nt_1sum,Y00_nt_1,Y01_nt_1,Y10_nt_1,Y11_nt_1,Y_nt_1,X_nt_1,Z_nt_1,G_nt_1,T_nt_1,file="datanet1new_5k.Rdata")
```
...
```{r}
YG1_nt_2<-0.5*Y1_ns_2 
YG0_nt_2<-0
YZ1_nt_2<-Y1_ns_2 # mean(X)
YZ0_nt_2<-Y0_ns_2
...
ps_nt_2_01 <- (1-ps_nt_2z)*ps_nt_2g
ps_nt_2_00 <- (1-ps_nt_2z)*(1-ps_nt_2g)
ps_nt_2sum <- data.frame(t11=ps_nt_2_11,t10=ps_nt_2_10,t01=ps_nt_2_01,t00=ps_nt_2_00,
                         z=ps_nt_2z,g=ps_nt_2g)


G_nt_2 <- sapply(ps_nt_2g,function(x){rbinom(1,1,prob=x)})
YG_nt_2 <- ifelse(G_nt_2==1,YG1_nt_2, 0)
Y_nt_2<-Y_ns_2+YG_nt_2 
#hist(YG1_nt_2)

# summarise and save data
X_nt_2<-X_ns_2
Z_nt_2<-T_ns_2 # Z_nt_1 <- sapply(ps_ns_1,function(x){rbinom(1,1,prob=x)})
T_nt_2<-paste0("t",Z_nt_2,G_nt_2)

# save(ps_nt_2sum,Y00_nt_2,Y01_nt_2,Y10_nt_2,Y11_nt_2,Y_nt_2,X_nt_2,Z_nt_2,G_nt_2,T_nt_2,file="datanet2new_5k.Rdata")
...
# actual outcome
#g3$YG <- ifelse(g3$G >0.5, YG1_nt_3, 0)
X_nt_3<-X_ns_2
Xnei_nt_3g <- get_Xnei(X_nt_3,A,"adj")
ps_nt_3g <- pnorm(Xnei_nt_3g[,1]) # closely centered around 0.5
G_nt_3 <- sapply(ps_nt_3g,function(x){rbinom(1,1,prob=x)})
g3$YG <- ifelse(G_nt_3==1, YG1_nt_3, 0)
Y_nt_3<-g3$YZ+g3$YG
...
save(Y00_nt_3,Y01_nt_3,Y10_nt_3,Y11_nt_3,Y_nt_3,X_nt_3,Z_nt_3,G_nt_3,T_nt_3,file="datanet3new_5k.Rdata")
```
...
#YG1_nt_4<-g4$YG
YG1_nt_4<-0.5*YZ_nt_4  # define YG independently from Xnei
YG0_nt_4<-0 
...
g4$YG <- ifelse(G_nt_4==1, YG1_nt_4, 0)
Y_nt_4<-g4$YZ+g4$YG 
...
```{r plot_coverage}
t6<-test6_cov[,1:4]
colnames(t6)<-c("1","2","3","4")
t6$test<-6
t6<- pivot_longer(t6,!test,names_to="Tr",values_to="cov")

t7<-test7_cov[,1:4]
colnames(t7)<-c("1","2","3","4")
t7$test<-7
t7<- pivot_longer(t7,!test,names_to="Tr",values_to="cov")

t8<-test8_cov[,1:4]
colnames(t8)<-c("1","2","3","4")
t8$test<-8
t8<- pivot_longer(t8,!test,names_to="Tr",values_to="cov")

t9<-test9_cov[,1:4]
colnames(t9)<-c("1","2","3","4")
t9$test<-9
t9<- pivot_longer(t9,!test,names_to="Tr",values_to="cov")

t10<-test10_global_cov[,1:4]
colnames(t10)<-c("1","2","3","4")
t10$test<-10
t10<- pivot_longer(t10,!test,names_to="Tr",values_to="cov")

testplot<-rbind.data.frame(t6,t7,t8,t9,t10)
...
par(mar=c(3,4,3,1))
boxplot(cov~Tr*test,data=testplot,boxwex=0.4,ylab="Length",
        main="Length of interval",
        col=c("slateblue1","lightblue","pink","tomato"),
        xaxt="n")
axis(1,at=seq(2.5,22,4),labels=c("test6","test7","test8","test9","test10"),tick=FALSE,cex=0.3)
for(i in seq(0.5 , 20 , 4)){ 
...
legend("topright", legend = c("spillover1","spillover0","direct1","direct0"), 
       col=c("slateblue1","lightblue","pink","tomato"),
       pch = 15, bty = "n", pt.cex = 2, cex = 1,  horiz = T, inset = c(0, 0))
```
