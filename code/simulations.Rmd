...
save(A_adj,Y00_nt_1,Y01_nt_1,Y10_nt_1,Y11_nt_1,Y_nt_1,X_nt_1,Z_nt_1,G_nt_1,T_nt_1,file="datanet1.Rdata")
```
...
plot(ps_ns_1,test1_gps1[,5],xlab="Z",ylab="Z estimate")  # pretty close
# check p(g=1)
...
test22wttest<-ps_est_ns_2[c(983,984,987,989:991,994,998:1000)]
test22wttest<-(1-test22wttest)/test22wttest
inds1<-which(T_ns_2==1)
ps_ns_2_tr <- ps_est_ns_2[inds1]
ps_ns_2_tr <-(1-ps_ns_2_tr)/ps_ns_2_tr
test2_pred2 <- conformalSplit(X=X_ns_2_tr, Y=Y1_ns_2_tr, test22Xtest,test22wttest,
                          outfun="QRF", 
                          wt=ps_ns_2_tr,CQR=TRUE,
...
## compute spatial PO (with interference) coverage
...
testid6 <- sample(1000,30) # n=30, run time: QRF 14s , QBART 110s
...
# coverage
cov1 <- abs(test6_PO[,2]-test6_PO[,1]) - max(abs(test6_Y [,1]-test6_PO[,1]),
                                            abs(test6_Y [,1]-test6_PO[,2]))
which(cov1>0) # 0% cover

cov2 <- abs(test6_PO[,4]-test6_PO[,3]) - max(abs(test6_Y [,2]-test6_PO[,3]),
                                            abs(test6_Y [,2]-test6_PO[,4]))
which(cov2>0) # 0% cover

cov3 <- abs(test6_PO[,6]-test6_PO[,5]) - max(abs(test6_Y [,3]-test6_PO[,5]),
                                            abs(test6_Y [,3]-test6_PO[,6]))
which(cov3>0) # 0% cover

cov4 <- abs(test6_PO[,8]-test6_PO[,7]) - max(abs(test6_Y [,4]-test6_PO[,7]),
                                            abs(test6_Y [,4]-test6_PO[,8]))
which(cov4>0) # 10% cover
