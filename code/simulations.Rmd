...
T_nt_1<-paste0("t",T_nt_1)
# testid0 694
X_nt_1Xtest <- X_nt_1[694,]
...
test6_pred1 <-conformal_ITE1(X=X_nt_1, Y=Y_nt_1, 
                              ...
                              outfun="RF", CQR=FALSE)
# spill1 -0.07336791	0.2483669
# spill0 0.7462095	0.8746951
# direct1 0.7625377	-0.5335482		
# direct0 0.05703968	0.09277987
```
...
compute_coverage<-function(X, Y,
  ...
  n<-length(Y)
    ...
    params<- list(A=A,Atype="adj",ps_pred_model="binomial",outfun="RF")
    if (experiment_id="test6"){
      for (i in 1:iter){
      testid0<-sample(n,1)
      results[i]<-do.call(conformal_ITE1,list(params,X=X_nt_1, Y=Y_nt_1,testid=testid0,
                                           tr=Z_nt_1,))
      }
    } else if (experiment_id="test7"){
      for (i in 1:iter){
      testid0<-sample(n,1)
      results[i]<-do.call(conformal_ITE1,list(params,X=X_nt_2, Y=Y_nt_2,testid=testid0,
                                           tr=Z_nt_2))
      }
    } else if (experiment_id="test8"){
      for (i in 1:iter){
      testid0<-sample(n,1)
      results[i]<-do.call(conformal_ITE1,list(params,X=X_nt_3, Y=Y_nt_3, testid=testid0,
                                           tr=Z_nt_3,))
      }
    }
