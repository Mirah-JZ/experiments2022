...
# uses same testid6
test7_res<-conformal_ITE1(X=X_nt_2, Y=Y_nt_2, 
                              testid=testid6,
                              tr=Z_nt_2,
                              A=A,Atype="adj",
                              ps_pred_model="multinom",
                              ...
                              outfun="QRF", CQR=TRUE,
                              retrainYmodel=TRUE)
test7_PO <- test7_res[[1]]
test7_Y <- data.frame(Y11=Y11_nt_2[testid6],
                      Y10=Y10_nt_2[testid6],
                      Y01=Y01_nt_2[testid6],
                      Y00=Y00_nt_2[testid6])
# coverage with estimated G and ps
cov1 <- abs(test7_PO[,2]-test7_PO[,1]) - pmax(abs(test7_Y [,1]-test7_PO[,1]),
                                            abs(test7_Y [,1]-test7_PO[,2]))
length(which(cov1>0))/length(cov1) # 97% cover

cov2 <- abs(test7_PO[,4]-test7_PO[,3]) - pmax(abs(test7_Y [,2]-test7_PO[,3]),
                                            abs(test7_Y [,2]-test7_PO[,4]))
length(which(cov2>0))/length(cov2) # 97% cover

cov3 <- abs(test7_PO[,6]-test7_PO[,5]) - pmax(abs(test7_Y [,3]-test7_PO[,5]),
                                            abs(test7_Y [,3]-test7_PO[,6]))
length(which(cov3>0))/length(cov3) # 93% cover

cov4 <- abs(test7_PO[,8]-test7_PO[,7]) - pmax(abs(test7_Y [,4]-test7_PO[,7]),
                                            abs(test7_Y [,4]-test7_PO[,8]))
length(which(cov4>0))/length(cov4) # 97% cover
```
...
test8_res<-conformal_ITE1(X=X_nt_4, Y=Y_nt_4, 
                              testid=testid6,
                              tr=Z_nt_4,
                              A=A,Atype="adj",
                              ps_pred_model="multinom",
                              ...
                              outfun="QRF", CQR=TRUE,
                              retrainYmodel=TRUE)
test8_PO <- test8_res[[1]]
test8_Y <- data.frame(Y11=Y11_nt_4[testid6],
                      Y10=Y10_nt_4[testid6],
                      Y01=Y01_nt_4[testid6],
                      Y00=Y00_nt_4[testid6])
# coverage with estimated G and ps
cov1 <- abs(test8_PO[,2]-test8_PO[,1]) - pmax(abs(test8_Y [,1]-test8_PO[,1]),
                                            abs(test8_Y [,1]-test8_PO[,2]))
length(which(cov1>0))/length(cov1) # 90% cover

cov2 <- abs(test8_PO[,4]-test8_PO[,3]) - pmax(abs(test8_Y [,2]-test8_PO[,3]),
                                            abs(test8_Y [,2]-test8_PO[,4]))
length(which(cov2>0))/length(cov2) # 93% cover

cov3 <- abs(test8_PO[,6]-test8_PO[,5]) - pmax(abs(test8_Y [,3]-test8_PO[,5]),
                                            abs(test8_Y [,3]-test8_PO[,6]))
length(which(cov3>0))/length(cov3) # 87% cover

cov4 <- abs(test8_PO[,8]-test8_PO[,7]) - pmax(abs(test8_Y [,4]-test8_PO[,7]),
                                            abs(test8_Y [,4]-test8_PO[,8]))
length(which(cov4>0))/length(cov4) # 93% cover
```
...
X_nt_5<-X_nt_4[,2:5]
test9_res<-conformal_ITE1(X=X_nt_5, Y=Y_nt_4, 
                              testid=testid6,
                              tr=Z_nt_4,
                              A=A,Atype="adj",
                              ps_pred_model="multinom",
                              ...
                              outfun="QRF", CQR=TRUE,
                              retrainYmodel=TRUE)
test9_PO <- test9_res[[1]]
# test9_Y is same as test8_Y
# coverage with estimated G and ps
cov1 <- abs(test9_PO[,2]-test9_PO[,1]) - pmax(abs(test8_Y [,1]-test9_PO[,1]),
                                            abs(test8_Y [,1]-test9_PO[,2]))
length(which(cov1>0))/length(cov1) # 90% cover

cov2 <- abs(test9_PO[,4]-test9_PO[,3]) - pmax(abs(test8_Y [,2]-test9_PO[,3]),
                                            abs(test8_Y [,2]-test9_PO[,4]))
length(which(cov2>0))/length(cov2) # 93% cover

cov3 <- abs(test9_PO[,6]-test9_PO[,5]) - pmax(abs(test8_Y [,3]-test9_PO[,5]),
                                            abs(test8_Y [,3]-test9_PO[,6]))
length(which(cov3>0))/length(cov3) # 83% cover

cov4 <- abs(test9_PO[,8]-test9_PO[,7]) - pmax(abs(test8_Y [,4]-test9_PO[,7]),
                                            abs(test8_Y [,4]-test9_PO[,8]))
length(which(cov4>0))/length(cov4) # 90% cover
```


```{r test10_global, eval=FALSE}
# test10 global. use conformal_ITE1()
# input X_nt_2, Y_nt_2, Z_nt_2, Y11_nt_2, Y10_nt_2, Y01_nt_2, Y00_nt_2
# uses same testid6
test10_res<-conformal_ITE1(X=X_nt_3, Y=Y_nt_3, 
                              testid=testid6,
                              tr=Z_nt_3,
                              A=A,Atype="adj",
                              ps_pred_model="multinom",
                              ...
                              outfun="QRF", CQR=TRUE,
                              retrainYmodel=TRUE)
test10_PO <- test10_res[[1]]
test10_Y <- data.frame(Y11=Y11_nt_3[testid6],
                      Y10=Y10_nt_3[testid6],
                      Y01=Y01_nt_3[testid6],
                      Y00=Y00_nt_3[testid6])
# coverage with estimated G and ps
cov1 <- abs(test10_PO[,2]-test10_PO[,1]) - pmax(abs(test10_Y [,1]-test10_PO[,1]),
                                            abs(test10_Y [,1]-test10_PO[,2]))
length(which(cov1>0))/length(cov1) # 87% cover

cov2 <- abs(test10_PO[,4]-test10_PO[,3]) - pmax(abs(test10_Y [,2]-test10_PO[,3]),
                                            abs(test10_Y [,2]-test10_PO[,4]))
length(which(cov2>0))/length(cov2) # 80% cover

cov3 <- abs(test10_PO[,6]-test10_PO[,5]) - pmax(abs(test10_Y [,3]-test10_PO[,5]),
                                            abs(test10_Y [,3]-test10_PO[,6]))
length(which(cov3>0))/length(cov3) # 97% cover

cov4 <- abs(test10_PO[,8]-test10_PO[,7]) - pmax(abs(test10_Y [,4]-test10_PO[,7]),
                                            abs(test10_Y [,4]-test10_PO[,8]))
length(which(cov4>0))/length(cov4) # 97% cover
```


```{r test10_local, eval=FALSE}
# test10 local. uses conformal_ITE2()
# input X_nt_2, Y_nt_2, Z_nt_2, Y11_nt_2, Y10_nt_2, Y01_nt_2, Y00_nt_2
# uses same testid6

test10_res<-conformal_ITE2(X=X_nt_3, Y=Y_nt_3, 
                              testid=testid6,
                              tr=Z_nt_3,
                              A=A,Atype="adj",
                              ps_pred_model="multinom",
