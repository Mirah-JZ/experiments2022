...
# wt        weights computed from gps, calculated at conformal_ITE() and passed in
...
conformalSplit <- function(X, Y, Xtest,wt_test,
  ...
  wt_cal <- wt[-trainid]
  avg_wt <- mean(c(wt_cal, wt_test))
  censoring <- function(x){pmin(pmax(x, 0.05), 20)}
  wt_cal <- censoring(wt_cal/avg_wt)
  wt_test <- censoring(wt_test/avg_wt)
  totw <- sum(wt_cal)
  #wt_cal <- wt_cal / totw
  qt <- (1 + wt_test / totw) * (1 - alpha) # quantile 1-alpha weighted
  qt <- pmin(qt, 1)
  ...
  if (CQR==FALSE){
    ...
    Ytest_hat <- predict(Ymodel, newdata = Xtest)
    ...
    Ytest_hat <-Ytest_hat$predictions
    Yslack <- weightedConformalCutoff(Yscore, wt_cal, qt) 
...
conformalCf_split <- function(X, Y, 
  ...
  inds<-list(inds11,inds10,inds01,inds00) # list of index, careful with the ordering
   ...
   # parse index
  X0<-Y0<-ps0<-out0<-trainid0<-testid0<-list()
  n0<-lengths(inds)
  minn<-min(n0)
  ...
  if(tltest=="t11"){
    out2 <- conformalSplit(X0[[2]], Y0[[2]],Xtest,wt_test,outfun,CQR,
                           wt=ps0[[2]][,1]/ps0[[2]][,2],trainid0[[2]])
    
    out3 <- conformalSplit(X0[[3]], Y0[[3]],Xtest,wt_test,outfun,CQR,
                           wt=ps0[[3]][,1]/ps0[[3]][,3],trainid0[[3]])
    
    out4 <- conformalSplit(X0[[4]], Y0[[4]],Xtest,wt_test,outfun,CQR,
                           wt=ps0[[4]][,1]/ps0[[4]][,4],trainid0[[4]])
    out<- list(data.frame(lower=Ytest,upper=Ytest),out2,out3,out4)
  } else if(tltest=="t10"){
    out1 <- conformalSplit(X0[[1]], Y0[[1]],  Xtest,wt_test,outfun,CQR,
                           wt=ps0[[1]][,2]/ps0[[1]][,1],trainid0[[1]])
    
    out3 <- conformalSplit(X0[[3]], Y0[[3]],  Xtest,wt_test,outfun,CQR,
                           wt=ps0[[3]][,2]/ps0[[3]][,3],trainid0[[3]])
    
    out4 <- conformalSplit(X0[[4]], Y0[[4]],  Xtest,outfun,CQR,
                           wt=ps0[[4]][,2]/ps0[[4]][,4],trainid0[[4]])
    out<- list(out1,data.frame(lower=Ytest,upper=Ytest),out3,out4)
  } else if(tltest=="t01"){
    out1 <- conformalSplit(X0[[1]], Y0[[1]],  Xtest,wt_test,outfun,CQR,
    ...
    out<- list(out1,out2,data.frame(lower=Ytest,upper=Ytest),out4)
  } else {
    out1 <- conformalSplit(X0[[1]], Y0[[1]],  Xtest,wt_test,outfun,CQR,
    ...
    out<- list(out1,out2,out3,data.frame(lower=Ytest,upper=Ytest)) # t00
  }
  #colnames(out)<-c("t11","t10","t01","t00")
  return(out)
}
...
# X, Y
# testid      index of Xtest point in full X
...
conformal_ITE1 <- function(X, Y, testid,
  ...
  if(tltest=="t11"){wt_test<- ps[testid,1]
  } else if (tltest=="t10") {wt_test<- ps[testid,2]
  } else if (tltest=="t01") {wt_test<- ps[testid,3]
  } else {wt_test<- ps[testid,4]
  }
  ...
  if(is.na(trainid)){
      ...
      #trainid <- sample(n,floor(n*0.6))
      Cf_params<-list(X,Y,Xtest,Ytest,tltest,wt_test,tl,outfun,CQR,ps=ps,trainid)
    }
  PO<-do.call(conformalCf_split,Cf_params) # get the four potential outcomes
  
  # compute ITEs
  # PO is a list
  ITE<-list(PO[[1]]-PO[[2]],PO[[3]]-PO[[4]],PO[[1]]-PO[[3]],PO[[2]]-PO[[4]])
  # needs to merge bounds a bit diff, otherwise good
  names(ITE)<-c("spill1","spill0","direct1","direct0")
