...
# gps estimation via binomial_logit+factorisation/multinomial_logit or binomial_logit
...
#             Conditional ps are for the Forastiere procedure.
...
get_gps1 <- function(tr,
  ...
  if (Atype=="dist") {
    ...
    # these lines need to be put into a function  get_trnei1(tr,A,Atype)
    A0 <- A
    diag(A0) <-1
    A0 <- A^(-1)
    diag(A0) <- 0
  ...
  if (pstype=="joint"){
    if (ps_pred_model=="multinom") {
         # nnet accepts formula, needs formatting
         data0=cbind.data.frame(covar0,tl)
         pred0 <- nnet::multinom(x=covar0,y=tl) 
         ps <- predict(pred0,newdata=data0,type="probs")
         # how are the cols ordered ?
         # insert code here, get marginals
         colnames(ps)<-c("t11","t10","t01","t00","z","g")
    }
    if (ps_pred_model=="binomial"){
         # p(Z|X_z)
         T_1 <- glm.fit(x=covar,y=tr,family="binomial")
         std1 <- T_1$sigma 
         pred1 <- T_1$fitted.values
         ps1 <- dnorm(tr,mean=pred1,sd=std1) 
         # p(Z|X_z,G)
         T_11 <- glm.fit(x=c(covar,trnei),y=tr,family="binomial")
         std11 <- T_11$sigma 
         pred11 <- T_11$fitted.values
         ps11 <- dnorm(tr,mean=pred11,sd=std11) 
         # p(G|X_g)
         T_2 <- glm.fit(x=covnei,y=trnei,family="binomial")
         std2 <- T_2$sigma 
         pred2 <- T_2$fitted.values
         ps2 <- dnorm(trnei,mean=pred2,sd=std2) 
         gps1 <- ps1*ps2 # z=1,g=1
         gps2 <- ps1*(1-ps2) # z=1,g=0 
         gps3 <- (1-ps1)*ps2 # z=0,g=1 
         gps4 <- (1-ps1)*(1-ps2) # z=0,g=0 
         ps<-cbind(gps1,gps2,gps3,gps4,ps1,ps2)
         colnames(ps)<-c("t11","t10","t01","t00","z","g")
    }
    else if (ps_pred_model=="xgboost") stop('xgboost multinom currently not supported.')
  }
  
  if (pstype=="conditional_g"){
    # P(G=g|Z=z,X=x)
    if (ps_pred_model=="xgboost"){
         ...
         ps <- dnorm(trnei,mean=pred,sd=std) 
    }    
  }
  if (pstype=="conditional_z"){
    # P(Z=z|G=g,X=x)
    if (ps_pred_model=="xgboost"){
         data0=cbind.data.frame(covar0,tr,trnei)
...
```{r conformalSplit}
# conformalSplit() is the base prediction function

# X, Y
# testid     testing point index.
# outfun    function for predicting Y
# outparams (if CQR quantiles should be part of outparams, not necessary for mean estimation)
# wt        gps weights already calculated at conformal_ITE() and passed in
# alpha     coverage 
# trainprop
# trainid   
...
conformalSplit <- functon(X, Y, testid,
                          outfun, outparams,
                          wt,
                          trainid,
                          alpha=0.1,
  ...
  Ytest_hat <- Ymodel(Xtest)
  
  avg_wt <- mean(c(wt_val, wt_test))
  wthigh <- 20 
  wtlow <- 0.05
  wt_val <- censoring(wt_val/avg_wt, wthigh, wtlow)
  ...
  qt <- pmin(qt, 1) 
  Yslack <- weightedConformalCutoff(Yscore, wt_val, qt) # find score(residual) at given qt
...
# X, Y
# testid      the index of data point to do prediction on, single point
# tl          vector of exposure levels
# A           spatial network A_ji, in the form of adjacency matrix or distance matrix 
# Atype       type of A, "dist" or "adj", 
# ps_pred_model passed to conformalSplit()
# outfun      passed to conformalSplit()
# outparams   passed to conformalSplit()
# wt          passed to conformalSplit()
# trainid     interface to other sampling methods (spatial).
...
conformalCf_split <- function(X, Y, 
  ...
  inds10 <- which(tl == "t10")  
  inds11 <- which(tl == "t11")
  inds<-list(inds00,inds01,inds10,inds11)
  
  # parse index
  ps<-1/ps
  
  n0<-x0<-y0<-ps0<-out0<-list()
  if (!is.null(trainid)){
    trainid0<-trainid
  } else { trainid0<-list()}
  ...
  for (i in 1:4){
    n0[i]<-length(inds[i])
    if (is.null(trainid)){
      trainid0[i]<- sample(n0[i], floor(n0[i] * trainprop))
    }
    x0[i] <- X[inds[i], ,drop=FALSE]
    Y0[i] <- Y[inds[i]]
    ps0[i]<- ps[inds[i]]
    out0[i] <- conformalSplit(x0[i], Y0[i], testid,outfun, outparams,ps0[i],trainid0[i])
  } 
  
  tltest<-tl[testid]
  if(tltest=="t00"){
    out<- list(Y[testid],out0[2],out0[3],out0[4])
  } if(tltest=="t01"){
    out<- list(out0[1],Y[testid],out0[3],out0[4])
  } if(tltest=="t10"){
    out<- list(out0[1],out0[2],Y[testid],out0[4])
  } else if (tltest=="t11"){
    out<- list(out0[1],out0[2],out0[3],Y[testid])
  }
  return(out)
}
...
# X, Y
# testid      the index of data point to do inference on, single point atm.
...
conformal_ITE1 <- function(X, Y, testid,
  ...
  ps<-get_gps1(tr,covar,A,Atype,pstype="joint",ps_pred_model="binomial") # dataframe n*6
  
  # compute counterfactuals
  trnei<-get_trnei1(tr,A,typeA)
  tl <- paste0("t",tr,trnei)
  tls <-c("t00","t01","t10","t11")
  Ztest <- tr[testid]
  Gtest <- trnei[testid]
  tltest <- paste0("t",Ztest,Gtest)
  
  Cf_params<-c(X,Y,testid,tl,outfun,outparams,wt=ps)
  PO <- list()
  for (i in 1:4){
    if (tls[i]!=tltest){
     PO[i]<- do.call(conformalCf_split,Cf_params)
   } else { PO[i]=Y[testid]}
  }
  colnames(as.data.frame(PO))<-c("t00","t01","t10","t11")
  
  # compute ITEs
  ITE<-data.frame(PO$t11-PO$t10,PO$t01-PO$t00,PO$t11-PO$t01,PO$t10-PO$t00)
  colnames(ITE)<-c("spill1","spill0","direct1","direct0")
  # average ITE over point testid's marginal treatment propensities
  ITE$spill_avg<-ITE$spill1/(ps$z[testid])+ITE$spill0/(1-ps$z[testid])
  ITE$direct_avg<-ITE$direct1/(ps$g[testid])+ITE$direct0/(1-ps$g[testid]) 
    
  # insert code here, check results
  return (ITE)
}
```
