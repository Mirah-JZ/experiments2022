...
conformalCf_split <- function(X, Y, 
  ...
  if ( length(test_grp11)!=0){
    ...
    out11<- data.frame()
    print ("No test ponits with tl=11")}
  ...
  if ( length(test_grp10)!=0) {
    ...
    out10<- data.frame()
    print ("No test ponits with tl=t10")}
  ...
  if ( length(test_grp01)!=0) {
    ...
    out01<- data.frame()
    print ("No test ponits with tl=t01")}
  ...
  if ( length(test_grp00)!=0) {
    ...
    out00<- data.frame()
    print ("No test ponits with tl=t00")}
...
```{r ITE_version1}
# in this version, testid is a vector of IDs. for each test point, the whole procedure is repeated including the ymodel fitting.
# difficult to pass whole vector of test points to prediction, because Ymodel training is conditional on the tl of the test point
# if the list of test points have different tl, it is difficult to parse.
# Therefore, even though terating over each test point takes longer to run, it is easier to implement...
...
conformal_ITE1 <- function(X, Y, testid,
  ...
  if (retrainYmodel==TRUE) {
     ...
     if(is.na(trainid)){
       ...
       } else {  print("If trainid not provided, please input NA.")
       }
